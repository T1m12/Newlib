local library = {
	...
	colors = {
		main = Color3.fromRGB(203, 63, 200)
				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./IyPlus")
							if fold then
							else
								makefolder("./IyPlus")
							end
							fold = isfolder("./IyPlus/Themes")
							if fold then
							else
								makefolder("./IyPlus/Themes")
							end
							fold = isfolder("./IyPlus/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./IyPlus Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./IyPlus/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./IyPlus/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./IyPlus/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
if isfolder('./IyPlus/Unbounded yield/Remember') == false then
	makefolder('./IyPlus/Unbounded yield/Remember')
end
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/Iratethisname10/Iy-plus/main/others/files/external2.lua"))()
do
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "IyPlus" then
		library.WorkspaceName = convertfilename(windowName, "IyPlus")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./IyPlus") then
						makefolder("./IyPlus")
					end
					local common_string = "./IyPlus/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./IyPlus/" .. tostring(custom_workspace or library.WorkspaceName)
						end
